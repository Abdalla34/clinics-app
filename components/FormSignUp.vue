<template>
  <div class="form-login">
    <form @submit.prevent="onSubmit">
      <div class="name mb-3">
        <label class="text-capitalize font-size label pb-1" for="">name</label>
        <input
          class="input"
          :class="{ 'is-valide': errors.name }"
          type="text"
          placeholder="input your registered email"
          v-model="name"
        />
        <p v-if="errors.name" class="text-capitalize text-danger font-12px">
          {{ errors.name }}
        </p>
      </div>
      <div class="email mb-3">
        ...
        <p v-if="emailHere" class="text-capitalize text-danger font-12px">
          this email already here
        </p>
      </div>
      ...
      <BtnLogin :class="{ 'background-active': email && password && name }" />
      ...
      <div class="create-acc d-flex justify-content-center mt-4 gap-1">
        ...
        <p
          ...
          class="text-capitalize color-main-text font-12px"
...
const schema = yup.object({
  ...
  name: yup
    .string("")
    .required("this is required")
    .min(3, "you must a lease 3 charcter longer "),
});
